...
__device__ void swap(CARD *c, CARD *c2)
...
__device__ void sort_hand(CARD *hand, int hand_size)
{
  /* bubble sort */ 
...
__device__ int distance(CARD c, CARD c2)
{
  if((c.rank == 12 || c2.rank == 12) && (c.rank == 3 || c2.rank == 3))
    return 1;

  int d = c.rank - c2.rank;
  if(d < 0)
    return d * -1;
  else
    return d;
...
__device__ int rank_hand(CARD *hand, BIN *bin, int size_of_hand)
      ...
      if(bin->is_full[8] == FALSE)
	{
	  if(bin->SF.b_count[c.suit] == 0) 
	    {/* if it is the first card with that suit, we want it in the bin */
	      AddMulti(c, bin->SF.b, bin->SF.b_max, c.suit, bin->SF.b_count, bin, 8);
	    }
	  if(bin->SF.b_count[c2.suit] == 0)
	    AddMulti(c2, bin->SF.b, bin->SF.b_max, c2.suit, bin->SF.b_count, bin, 8);
		  ...
		  if(current_count > 0)
			...
			{/*last card in bin is a two...need to check for an Ace in the front to make the wheel*/
			  ...
			  while(hand[cnt].rank == 12)
				...
				{
				  AddMulti(hand[cnt], bin->SF.b, bin->SF.b_max, hand[cnt].suit, bin->SF.b_count, bin, 8);
				}
			      cnt++;
			    }
			}
      ...
      if(bin->is_full[5] == FALSE)
      	{
      	  if(i % 2 == 0)
      	    {/* add on the evens otherwise we would count every card twice */
      	      ...
      	      AddMulti(c, bin->F.b, bin->F.b_max, c.suit, bin->F.b_count, bin, 5);
      	      if(bin->F.b_count[c2.suit] < bin->F.b_max)
      		{/* we can add second card */
      		  AddMulti(c2, bin->F.b, bin->F.b_max, c2.suit, bin->F.b_count, bin, 5);
      		}
      	    }/* END if(i % 2 == 0) */
      	  else
	  ...
	  if(temp.suit == temp2.suit)
		...
		{/* if the bin isn't full AND has something in it and first and last card are Ace and Two */
		  ...
		  if(bin->SF.b[temp.suit][0].rank == 3)
		    {/* Highest Card in the Straight bin HAS to be 5...3 is 5 btw...rank starts at 0 which is 2 */
		      AddMulti(temp, bin->SF.b, bin->SF.b_max, temp.suit, bin->SF.b_count, bin, 8);
		    }
		}
	    }
